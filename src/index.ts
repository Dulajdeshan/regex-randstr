...
const generateFromTokens = (tokens: Token[]): string => {
    ...
    while (i < tokens.length) {
        ...
        switch (token.type) {
            case 'dot':
                result += getRandomCharFromCharset('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'.split(''));
                i++;
                break;
            ...
            case 'class':
                ...
                if (tokens[i + 1] && tokens[i + 1].type === 'quantifier') {
                    ...
                    } else if (typeof quantifierToken.value === 'string' && quantifierToken.value.startsWith('{')) {
 ...
 */
const randString = (regex: string | RegExp): string => {
